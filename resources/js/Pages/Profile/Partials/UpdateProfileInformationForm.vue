...
<template>
    <div class="space-y-8 px-4 sm:px-0">
      ...
      <div class="text-center mb-2">
        ...
        <h1 class="text-2xl font-bold text-gray-800 mb-2">Profil Kita</h1>
      ...
      <div class="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-white/50 overflow-hidden">
        ...
        <div class="relative bg-gradient-to-r from-pink-500/10 to-rose-500/10 py-6 px-4 text-center border-b border-pink-100">
          <div class="relative inline-block">
            ...
            <div class="relative group cursor-pointer" @click="togglePhotoDropdown">
              ...
              <div class="relative">
                <img 
                  ...
                  :alt="user.name" 
                  class="w-40 h-40 rounded-full object-cover border-4 border-white shadow-2xl mx-auto transition-all duration-300 group-hover:scale-105 group-hover:border-pink-200"
                />
                <!-- Overlay Hover -->
                <div class="absolute inset-0 bg-black/20 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                  <span class="text-white text-sm font-medium bg-black/50 px-3 py-1 rounded-full">Ubah Foto</span>
                </div>
              </div>
              ...
              <div class="absolute -bottom-2 -left-2 w-6 h-6 bg-rose-300 rounded-full flex items-center justify-center shadow-lg">
                <span class="text-white text-xs">❤️</span>
              </div>
            </div>

            <!-- User Name -->
            <h2 class="text-2xl font-bold text-gray-800 mt-6 mb-2">{{ user.name }}</h2>
            <p class="text-gray-600 flex items-center justify-center gap-2">
              <span class="text-pink-400">📧</span>
              {{ user.email }}
              <span v-if="user.email_verified_at" class="text-green-500 text-sm flex items-center gap-1">
                ✓ Terverifikasi
              </span>
            </p>
            ...
            <div 
              ...
              class="absolute top-full left-1/2 transform -translate-x-1/2 mt-4 w-48 bg-white rounded-xl shadow-2xl border border-gray-100 z-50 overflow-hidden"
            >
              <button 
                @click="selectNewPhoto"
                class="w-full px-4 py-3 text-left hover:bg-pink-50 transition-colors flex items-center gap-3 text-gray-700"
              ...
              <button 
                @click="showFullPhoto"
                class="w-full px-4 py-3 text-left hover:bg-pink-50 transition-colors flex items-center gap-3 text-gray-700 border-t border-gray-100"
              >
                <span class="text-lg">👁️</span>
              ...
              <button 
                ...
                @click="deletePhoto"
                class="w-full px-4 py-3 text-left hover:bg-red-50 transition-colors flex items-center gap-3 text-red-600 border-t border-gray-100"
                :disabled="form.processing"
              >
                <span class="text-lg">🗑️</span>
        ...
        <div class="p-4">
          ...
          <div class="mb-4">
            ...
            <div class="relative">
              <input
                ...
                v-model="form.name"
                type="text"
                class="w-full border border-gray-200 rounded-2xl p-2 pl-12 focus:ring-2 focus:ring-pink-300 focus:border-pink-300 transition-all duration-300 bg-white/50 backdrop-blur-sm"
                required
                autocomplete="name"
                :disabled="form.processing"
            ...
            <p v-if="form.errors.name" class="mt-2 text-sm text-red-500 flex items-center gap-2">
              <span>⚠️</span>
              {{ form.errors.name }}
            </p>
          </div>
          ...
          <div class="mb-4">
            ...
            <p v-if="form.errors.email" class="mt-2 text-sm text-red-500 flex items-center gap-2">
              <span>⚠️</span>
              {{ form.errors.email }}
            </p>

            <!-- Email Verification -->
            <div v-if="$page.props.jetstream.hasEmailVerification && user.email_verified_at === null" class="mt-4 p-4 bg-amber-50/80 border border-amber-200 rounded-2xl">
              <div class="flex items-start gap-3">
                <span class="text-amber-500 text-xl mt-1">⚠️</span>
                <div class="flex-1">
                  <p class="text-amber-800 font-medium mb-2">Email Belum Terverifikasi</p>
                  ...
                  <Link
                    ...
                    method="post"
                    as="button"
                    class="inline-flex items-center gap-2 px-4 py-2 bg-amber-100 text-amber-800 rounded-xl hover:bg-amber-200 transition-all duration-300 font-medium text-sm border border-amber-300"
              ...
              <div v-show="verificationLinkSent" class="mt-2 p-2 bg-green-100 border border-green-300 rounded-xl">
          ...
          <!-- Photo Error -->
          <p v-if="form.errors.photo" class="mb-6 p-4 bg-red-50 border border-red-200 rounded-2xl text-red-600 text-sm flex items-center gap-2">
            <span>❌</span>
            {{ form.errors.photo }}
          </p>
    ...
    <!-- Photo Modal -->
    <div v-if="showPhotoModal" class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-hidden shadow-2xl">
        <div class="relative">
          <button 
            @click="closePhotoModal"
            class="absolute top-4 right-4 w-10 h-10 bg-black/50 text-white rounded-full flex items-center justify-center hover:bg-black/70 transition-colors z-10"
          >
            ✕
          </button>
          <img 
        ...
        <div class="p-6 bg-gradient-to-r from-pink-50 to-rose-50 text-center">
          <p class="text-gray-600">Foto Profil {{ user.name }}</p>
        </div>
      </div>
    </div>
</template>